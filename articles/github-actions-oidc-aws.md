...
本記事では、扱う各技術・サービスについての説明は詳細には行いません。
...
`リポジトリにある s3/index.html ファイルを、S3 バケットにアップロードする`
というデモを行います。

## 手順

GitHub のドキュメントに沿って進めました。
...
2. AWS | IAM ロール作成
3. AWS | S3 バケットを作成
4. AWS | IAM ポリシーを作成し IAM ロールにアタッチ
...
プロバイダ URL を入力した後に、「サムプリントを取得」を押してください。
...
ここで、どういう条件でこのロールを引き受けられるかを定義できているので、セキュアとなります。

:::details 条件を満たさない場合の workflow のテスト

異なるリポジトリ上で Assume Role を実行したところ、以下の画像の通りエラーとなりました。
...
- 設定: デフォルト (バージョニングだけなんとなく ON しました)
...
1 で作成した IAM ロールにアタッチする IAM ポリシーを作成します。

- ポリシー名: `S3PutObjectForOIDCGitHubActionsExamplePolicy`
- 権限: my-oidc-example に index.html をアップロードできる
...
特に、「これってみんなこのロールが使えちゃうんじゃないの？」という素朴な疑問なども、ドキュメント内で解消されている点が、とても素晴らしいと思いました。

今後はこれらのリソースを Terraform 等 IaC で構築できるように勉強していきたいです！
